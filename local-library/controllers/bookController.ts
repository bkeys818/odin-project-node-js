...
export const bookCreatePost = [
    ...
    asyncHandler(async (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            const checked_genres: (typeof book._id)[] = []
            for (const genre of allGenres) {
                if (book.genre?.includes(genre._id))
                    checked_genres.push(genre._id)
            }
            res.render('book-form', {
                title: 'Create Book',
                authors: allAuthors,
...
export const bookDeletePost = asyncHandler(async (req, res, next) => {
    const [book, allBooksByBook] = await Promise.all([
        Book.findById(req.params.id).exec(),
        Book.find({ book: req.params.id }, 'title summary').exec(),
    ])

    if (allBooksByBook.length > 0) {
        res.render('book-delete', {
            title: 'Delete Book',
            book: book,
            book_books: allBooksByBook,
        })
    } else {
...
export const bookUpdateGet = asyncHandler(async (req, res, next) => {
    ...
    const checked_genres: (typeof book._id)[] = []
    for (const genre of allGenres) {
        if (book.genre?.includes(genre._id)) checked_genres.push(genre._id)
    }
